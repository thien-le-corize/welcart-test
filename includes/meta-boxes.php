...
function usces_post_categories_meta_box( $post, $box ) {
	...
	<div id="taxonomy-<?php echo esc_attr( $taxonomy ); ?>" class="categorydiv">
		<ul id="<?php echo esc_attr( $taxonomy ); ?>-tabs" class="category-tabs">
			<li class="tabs"><a href="#<?php echo esc_attr( $taxonomy ); ?>-all" tabindex="3"><?php _e( 'Item Category', 'usces' ); ?></a></li>
			<li class="hide-if-no-js"><a href="#<?php echo esc_attr( $taxonomy ); ?>-pop" tabindex="3"><?php _e( 'Most Used', 'usces' ); ?></a></li>
		</ul>
...
?>
<label class="screen-reader-text" for="excerpt"><?php _e('Excerpt') ?></label><textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt"><?php wel_esc_script_e( $post->post_excerpt ); ?></textarea>
...
?>
<p><label for="trackback_url"><?php _e('Send trackbacks to:'); ?></label><?php wel_esc_script_e( $form_trackback ); ?></p>
...
function usces_post_comment_meta_box( $post ) {
	...
	?>
	<p class="hide-if-no-js" id="add-new-comment"><a class="button" href="#commentstatusdiv" onclick="window.commentReply && commentReply.addcomment(<?php echo esc_js( $post->ID ); ?>);return false;"><?php _e('Add comment'); ?></a></p>
	<?php
	...
	if ( 1 > $total ) {
		...
		?>
		<p class="hide-if-no-js" id="show-comments"><a href="#commentstatusdiv" onclick="commentsBox.load(<?php echo esc_js( $total ); ?>);return false;"><?php _e('Show comments'); ?></a> <span class="spinner"></span></p>
		<?php
	}
...
<?php if ( !empty($link->link_id) ) { ?>
	<a class="preview button" href="<?php echo esc_url( $link->link_url ); ?>" target="_blank" tabindex="4"><?php _e('Visit Link'); ?></a>
<?php } ?>
</div>
<div class="clear"></div>
</div>
 ...
 */
function usces_xfn_check( $class, $value = '', $deprecated = '' ) {
	global $link;
